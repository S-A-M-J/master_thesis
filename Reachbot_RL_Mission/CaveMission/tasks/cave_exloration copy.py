...
class CaveExplore(reachbot_base.ReachbotEnv):
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    if self._config.stickiness_config.enable:
      ...
      xfrc_applied = self._apply_stickiness_forces(state, action)
  ...
  def _apply_stickiness_forces(self, state: mjx_env.State, action: jax.Array) -> jax.Array:
    ...
    # Get stickiness control signals from the action
    # Assuming the last N elements of the action vector control stickiness for each boom
    # (where N is the number of booms)
    num_booms = 4  # Adjust based on your robot configuration
    stickiness_signals = action[-num_booms:]  # Last elements of action vector
    ...
    for i, (boom_id, wall_id) in enumerate(zip(boom_idxs, wall_idxs)):
      ...
      boom_num = int(boom_name.split('boomEnd')[1]) if len(boom_name.split('boomEnd')) > 1 else i % num_booms
      
      # Check if stickiness is activated for this boom (threshold > 0.5)
      if boom_num < len(stickiness_signals) and stickiness_signals[boom_num] > 0.5:
        # Get positions
  ...
  def _get_reward(
    ...
    del metrics  # Unused.
    return {
        "range_from_target": self._cost_tracking(
            data.qpos[0:3], info["target"]
        ),
