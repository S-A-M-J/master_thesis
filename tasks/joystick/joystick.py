...
from models.model_loader import ReachbotModel
...
def default_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ...
      reward_config = config_dict.create(
        scales=config_dict.create(
            # Tracking rewards
            tracking_lin_vel=2.0,       # Reward scale for tracking linear velocity. Default is 1.0
            tracking_ang_vel=1.0,       # Reward scale for tracking angular velocity. Default is 0.5
            
            # Base movement rewards
            lin_vel_z=-0.5,             # Penalty scale for linear velocity in the z-axis
            ang_vel_xy=-0.05,           # Penalty scale for angular velocity in the x and y axes
            ...
            feet_clearance=0.0,        # Penalty scale for feet clearance. Default is -2.0
            feet_height=-0.2,           # Penalty scale for feet height. Default is -0.2
            feet_slip=-0.1,             # Penalty scale for feet slipping. Default is -0.1
            feet_air_time=0.1,          # Reward scale for feet air time. Default is 0.1
            correct_height=-0.0,         # Penalty scale for incorrect torso height. Added for Reachbot
            boom_extension_speed=-0.01,  # Penalty scale for boom extension speed. Default is -0.0
        ),
        tracking_sigma=0.25,            # Standard deviation for tracking reward calculation
        max_foot_height=0.3,              # Maximum foot height, changed from 0.1 to 1 for Reachbot
      ),
...
class Joystick(reachbot_base.ReachbotEnv):
  ...
  def __init__(
    ...
    self.reachbot_model = ReachbotModel(model)
    template_path = os.path.join(os.path.dirname(__file__), "environment/joystick_scene_template.xml")
    with open(template_path, "r") as f:
        scene_template = f.read()
    scene_xml = scene_template.replace("{REACHBOT_MODEL_PATH}", self.reachbot_model.model_path)
    xml_file = os.path.join(os.path.dirname(__file__), "environment/joystick_scene.xml")
    with open(xml_file, "w") as f:
      f.write(scene_xml)
    self._scene_xml = xml_file
    super().__init__(
        xml_path=self._scene_xml,
        config=config,
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    state_formatted = self._qpos_to_ctrl(self._init_q)
  ...
  def _get_reward(
    ...
    gate = self._at_correct_height(data.site_xpos[self._imu_site_id][2])
    del metrics  # Unused.
    return {
        "tracking_lin_vel": self._reward_tracking_lin_vel(
            info["command"], self.get_local_linvel(data), gate
        ),
        "tracking_ang_vel": self._reward_tracking_ang_vel(
            info["command"], self.get_gyro(data), gate
        ),
        ...
        "correct_height": self._cost_correct_height(data.site_xpos[self._imu_site_id][2]),
  ...
  def _reward_tracking_lin_vel(
    ...
    return jp.exp(-lin_vel_error / self._config.reward_config.tracking_sigma) * gate
  ...
  def _reward_tracking_ang_vel(
    ...
    return jp.exp(-ang_vel_error / self._config.reward_config.tracking_sigma) * gate
  ...
  def _cost_correct_height(self, torso_height: jax.Array) -> jax.Array:
    """Returns a penalty value if the torso height is outside a threshold range.
    
    The threshold range is defined as a fraction (low_height_threshold_factor)
    of the desired height (self._z_des). If torso_height is outside this range,
    the penalty is proportional to the difference.
    """
    tolerance = 0.05
    # Compute the penalty for being too low or too high.
    error = jp.abs(jp.abs(torso_height - self._z_des) - tolerance) * 4
    return jp.exp(-error) + 1.0
  
  def _at_correct_height(self, torso_height: jax.Array) -> jax.Array:
    """Returns a boolean value indicating if the torso is at the correct height."""
    return jp.abs(torso_height - self._z_des) < 0.05
  ...
  def action_size(self) -> int:
    ...
    joints = self._mj_model.nu  # Exclude the first 6 joints (free joint and deflection joints).
