...
def generate_cave_tunnel(curve_intensity=0.0, seed=None):
    ...
    offset_magnitude = 0  # No random offsets
    num_dents = 0  # No dents
    
    # No dented segments since we want no dents
    dented_segments = []
    
    #print(f"Adding dents to segments: {dented_segments}")
    
    # Generate straight centerline with no offsets
    t = np.linspace(0, length, num_segments)
    centerline = np.zeros((num_segments, 3))
    centerline[:, 0] = t  # Base X coordinates - straight line
    centerline[:, 1] = 0  # No Y curvature
    centerline[:, 2] = 0  # No Z offset
    
    # Keep centerline perfectly straight - no modifications
    ...
    for i in range(num_segments):
        ...
        # Create regular circular cross-section (no dents)
...
def create_cave(cave_id, output_dir, curve_intensity=0.0, voxel_size=0.2, seed=None):
...
if __name__ == "__main__":
    ...
    # Generate only one cave as requested
    for i in tqdm(range(1), desc="Generating cave", unit="cave"):
        cave_id = i + 1
        ...
        # Generate straight cave with no curvature
        curve_intensity = 0.0  # No curvature
    ...
    summary_data = {
        "generation_info": {
            "base_seed": args.seed,
            "cave_count": 1,  # Always generate one cave
    ...
    print(f"\nGenerated 1 straight cave with no dents, voxel size {args.voxel_size}m using seed {args.seed}. Summary saved to {summary_path}")
