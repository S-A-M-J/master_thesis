"""
Generate cave tunnels for MuJoCo using procedural generation with configurable parameters.
Each cave is saved as a voxelized mesh and a JSON configuration file with box positions.
"""
...
def generate_cave_tunnel(length=20.0, curve_intensity=0.5, num_dents=3, dent_depth_range=(0.4, 1.0), dent_width_range=(np.pi/6, np.pi/2), offset_magnitude=2, seed=None):
    ...
    # Parameters
    radius = 1.5  # meters
    num_segments = int(np.ceil(length / 2))
    points_per_circle = 12
    
    # Handle case where no dents are wanted
    dented_segments = []
    dent_parameters = []  # Track specific dent parameters used
    ...
    if curve_intensity > 0:
        ...
        # Scale the divisor based on length to maintain consistent curve shape
        curve_divisor = length * 1.25  # Scales with length (was 25 for length=20)
        centerline[:, 1] = curve_direction * curve_intensity * (t - t[0]) * (t - t[-1]) / curve_divisor
    ...
    for i in range(num_segments):
        ...
        if i in dented_segments:
            ...
            # Store the specific dent parameters used
            dent_parameters.append({
                "segment": int(i),
                "center": float(dent_center),
                "width": float(dent_width),
                "depth": float(dent_depth)
            })
...
def create_cave(cave_id, output_dir, length=20.0, curve_intensity=0.5, num_dents=3, dent_depth_range=(0.4, 1.0), dent_width_range=(np.pi/6, np.pi/2), offset_magnitude=2, voxel_size=0.2, seed=None):
    ...
    radius = 2.0  # meters
    # Scale number of segments based on length to maintain tunnel smoothness
    # Use approximately 0.5 segments per meter (minimum 10, maximum reasonable limit)
    num_segments = max(10, min(int(length * 0.5), 200))
    points_per_circle = 12
    ...
    cave_mesh, actual_dent_parameters = generate_cave_tunnel(length, curve_intensity, num_dents, dent_depth_range, dent_width_range, offset_magnitude, seed)
    ...
    # Update generation parameters with actual values used
    cave_params["generation"]["actual_dent_parameters"] = actual_dent_parameters
...
if __name__ == "__main__":
    ...
    parser.add_argument("--length", type=float, default=20.0, help="Cave length in meters (default: 20.0)")
    parser.add_argument("--curve-intensity-min", type=float, default=0, help="Minimum curve intensity for cave path (default: 0)")
    parser.add_argument("--curve-intensity-max", type=float, default=0.5, help="Maximum curve intensity for cave path (default: 0.5)")
    parser.add_argument("--num-dents-min", type=int, default=1, help="Minimum number of dents to add to cave walls (default: 1)")
    parser.add_argument("--num-dents-max", type=int, default=3, help="Maximum number of dents to add to cave walls (default: 3)")
    ...
    parser.add_argument("--offset-magnitude-min", type=float, default=0, help="Minimum random offset magnitude for cave path (default: 0)")
    parser.add_argument("--offset-magnitude-max", type=float, default=1.5, help="Maximum random offset magnitude for cave path (default: 1.5)")
    ...
    for i in tqdm(range(args.count), desc="Generating caves", unit="cave"):
        ...
        # Generate caves with different parameters using the cave-specific seed
        np.random.seed(cave_seed)
        
        # Sample curve intensity from the specified range
        curve_intensity = np.random.uniform(args.curve_intensity_min, args.curve_intensity_max)
        
        # Sample number of dents from the specified range
        num_dents = np.random.randint(args.num_dents_min, args.num_dents_max + 1)
        
        # Sample offset magnitude from the specified range
        offset_magnitude = np.random.uniform(args.offset_magnitude_min, args.offset_magnitude_max)
    ...
    summary_data = {
        "generation_info": {
            ...
            "length": args.length,
            "curve_intensity_range": [args.curve_intensity_min, args.curve_intensity_max],
            "num_dents_range": [args.num_dents_min, args.num_dents_max],
            "dent_depth_range": [args.dent_depth_min, args.dent_depth_max],
            "dent_width_range": [args.dent_width_min, args.dent_width_max],
            "offset_magnitude_range": [args.offset_magnitude_min, args.offset_magnitude_max],
    ...
    print(f"\nGenerated {args.count} caves with voxel size {args.voxel_size}m using seed {args.seed}.")
    print(f"Length of caves: {args.length} meters")
    print(f"Curve intensity range: [{args.curve_intensity_min}, {args.curve_intensity_max}]")
    print(f"Number of dents range: [{args.num_dents_min}, {args.num_dents_max}]")
    print(f"Offset magnitude range: [{args.offset_magnitude_min}, {args.offset_magnitude_max}]")
