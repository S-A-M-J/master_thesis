...
class CaveExplore(mjx_env.MjxEnv):
  """Explore cave task.
    ...
    Initializes the cave exploration environment.
    Args:
        config (config_dict.ConfigDict, optional): 
            The base configuration for the environment. If not provided, uses the default configuration.
        config_overrides (Optional[Dict[str, Union[str, int, list[Any]]]], optional): 
            Dictionary of configuration overrides to update the base config.
        cave_batch_loader (Optional[Any], optional): 
            Loader object to fetch scene data if not provided directly.
        scene_type (str, optional): 
            Type of scene to load (e.g., "training", "validation", "test"). Default is "training".
        scene_data (Optional[Dict[str, Any]], optional): 
            Directly provided scene data dictionary. If given, cave_batch_loader is ignored.
    Raises:
        ValueError: If neither cave_batch_loader nor scene_data is provided.
    Initializes:
        - Environment configuration and overrides.
        - Scene data and cave parameters.
        - Master cave information and geometry mapping.
        - LIDAR parameters and update intervals.
        - Milestone reward parameters.
        - Domain randomization data (if enabled).
        - Calls parent class initializer and post-initialization.
    """
  ...
  def _post_init(self) -> None:
    self._init_q = jp.array(self._scene_data["mj_model"].keyframe("stand").qpos)
    self._default_pose = jp.array(self._scene_data["mj_model"].keyframe("stand").qpos[7:])
    ...
    self._torso_geom_id = self._scene_data["mj_model"].geom("mainBody").id
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    new_position = jp.array([
        0.0, 0.0, 0.4  # Default starting position if no cave selected
    ])
  ...
  def get_collisions_with_boom_ends(
    ...
    dists_b = jp.where(mask, dist_arr[None, :], large_distance)  # [M,N]
  ...
  def _cost_stability(self, boom_contact_dists: jax.Array, local_feet_pos: jax.Array, info: Dict[str, Any] = None) -> jax.Array:
      ...
      in_contact = boom_contact_dists <= 0.05  # (4,)
      ...
      # Mask feet positions that are below torso height (z < 0)
      feet_below_torso = local_feet_pos[:, 2] < 0.

      # If a foot is not below torso, it cannot provide support
      in_contact = in_contact | feet_below_torso
