...
def domain_randomize(model: mjx.Model, cave_wall_geom_ids: jax.Array, box_positions: jax.Array, rng: jax.Array):
    """Legacy domain randomization function for backward compatibility."""
    
    @jax.vmap
    def rand_dynamics(rng):
        # Update box positions
        geom_pos = model.geom_pos
        num_wanted_boxes = box_positions.shape[0]
        num_cave_geoms = cave_wall_geom_ids.shape[0]
        
        # Use minimum to avoid index out of bounds
        boxes_to_place = jp.minimum(num_wanted_boxes, num_cave_geoms)
        
        # Place the wanted boxes at their specified positions
        geom_pos = geom_pos.at[cave_wall_geom_ids[:boxes_to_place]].set(
            box_positions[:boxes_to_place]
        )
        
        # Move leftover boxes to z=-1000 (hide them underground)
        leftover_count = num_cave_geoms - boxes_to_place
        
        def move_leftover_boxes():
            leftover_positions = jp.tile(jp.array([0.0, 0.0, -1000.0]), (leftover_count, 1))
            return geom_pos.at[cave_wall_geom_ids[boxes_to_place:]].set(leftover_positions)
    ...
    jax.debug.print("Domain randomization applied with {num_boxes} boxes", num_boxes=box_positions.shape[0])
